...
- Added proper error handling for MT5 operations
...
- OPTIMIZED: Reduced memory usage during predictions
- OPTIMIZED: Better TensorFlow graph mode configuration
- BUGFIX: Fixed variable naming conflict between TensorFlow import and timeframe variables
"""

# --- CONFIGURATION: MT5 PATH NOW LOADED FROM config.json ---
# The MT5 Files path is now loaded from config.json at runtime
# No need to edit this file - use setup_wizard.bat or the GUI to configure
MT5_FILES_PATH = None  # This will be loaded by config_manager
...
# Import configuration manager for MT5 path
try:
    ...
    print("=" * 80)
    print()
    print("The config_manager.py file must be in the same directory as this script.")
    print("This file is required for loading the MT5 Files path from config.json")
    print()
    print("If you just upgraded from v1.0/1.1:")
    print("  1. Make sure config_manager.py is in this folder")
    print("  2. Run setup_wizard.bat to create config.json")
    print()
    print("If you're installing fresh:")
    print("  1. Use the installer which includes all files")
    print("  2. Or manually copy config_manager.py to this folder")
    print()
    sys.exit(1)
...
def install_package(package_name: str, pip_name: Optional[str] = None) -> None:
    ...
    Args:
        ...
        pip_name: Name used for pip installation (if different from package_name)
    """
    try:
        ...
        try:
            ...
            print(f"ERROR: Failed to install {install_name}: {e}")
            sys.exit(1)

# FIXED: Correct package names for pip installation
required_packages = [
    ...
    ('lightgbm', 'lightgbm'),
    ('keras_tuner', 'keras-tuner')  # FIXED: Separate import name from pip name
]
...
import tensorflow as tf  # still used for seeds / convert_to_tensor / config
...
# Configure TensorFlow to reduce retracing and improve performance
tf.config.run_functions_eagerly(False)  # Use graph mode for better performance

# --- Helper Classes ---
class KalmanFilter:
    """Simple Kalman Filter for smoothing predictions."""
    ...
    def __init__(self, process_variance: float, measurement_variance: float):
        ...
        Args:
            process_variance: Q - how much predictions can change
            measurement_variance: R - trust in raw predictions
        """
...
class TransformerBlock(layers.Layer):
    """Transformer block for time series prediction."""
...
class UnifiedLSTMPredictor:
    """Main predictor class for hybrid ensemble MT5 forecasting."""
    ...
    def __init__(self, symbol: str = "EURUSD", related_symbols: Optional[List[str]] = None, 
        ...
        Args:
            ...
            related_symbols: List of related symbols for correlation analysis
            ensemble_model_types: List of model types to use in ensemble
            use_kalman: Whether to use Kalman filtering for smoothing
            use_multitimeframe: Whether to use separate models per timeframe (recommended)
        """
        self.symbol = symbol
        self.related_symbols = related_symbols or []
        self.ensemble_model_types = ensemble_model_types if ensemble_model_types is not None else []
        ...
        # Kalman filter configuration for log returns
        self.kalman_config = {
            "1H": {"Q": 0.00001, "R": 0.01},   # Very stable for short term
            "4H": {"Q": 0.00005, "R": 0.02},   
            "1D": {"Q": 0.0001, "R": 0.05}     
        }
        self.kalman_filters = {
            timeframe: KalmanFilter(config["Q"], config["R"]) 
            for timeframe, config in self.kalman_config.items()
        }
        
        # EMA smoothing as backup
        self.previous_predictions: Dict[str, Optional[float]] = {timeframe: None for timeframe in self.kalman_config.keys()}
        self.ema_alpha = 0.3
        
        # Ensemble weighting
        self.ensemble_weights = [1.0 / self.num_ensemble_models] * self.num_ensemble_models if self.num_ensemble_models > 0 else []
        self.prediction_history: Dict[str, List] = {timeframe: [] for timeframe in self.kalman_config.keys()}
        self.ensemble_lookback = 20
        self.ensemble_learning_rate = 0.1
        
        self.initialize_mt5()
        print(f"Hybrid Ensemble Predictor Initialized for {self.symbol} (v6.5.2 - Stable)")
        if self.use_multitimeframe:
            print("Multi-timeframe mode: ENABLED (separate models per timeframe)")
        else:
            print("Multi-timeframe mode: DISABLED (using scaling - not recommended)")
        if self.use_kalman:
            print("Kalman filtering: ENABLED (applied to log returns)")
        else:
            print("Kalman filtering: DISABLED (using EMA smoothing)")
        if self.ensemble_model_types:
            print(f"Ensemble configuration: {self.ensemble_model_types}")

    def get_mt5_files_path(self) -> str:
        """Verify and return MT5 files path from config.json."""
        try:
            ...
            if not os.path.exists(mt5_path):
                print(f"ERROR: MT5 Files directory does not exist: {mt5_path}")
                print()
                print("Please update config.json with the correct path or run setup_wizard.bat")
                sys.exit(1)
            
            # Test write permissions
            try:
                test_file = os.path.join(mt5_path, '.write_test')
                with open(test_file, 'w') as f:
                    f.write('test')
                os.remove(test_file)
            except Exception as e:
                print(f"ERROR: MT5 Files Path is not writable: {e}")
                print(f"Path: {mt5_path}")
                print()
                print("Please check folder permissions or run MetaTrader 5 as administrator once")
                sys.exit(1)
            
            # Ensure directory exists (shouldn't need this but just in case)
            ...
            print("=" * 80)
            print("ERROR: MT5 Files Path Not Configured!")
            print("=" * 80)
            print()
            print(str(e))
            print()
            print("To configure your MT5 path, you can:")
            print("  1. Run setup_wizard.bat (interactive setup)")
            print("  2. Use the GUI and click 'Save MT5 Path'")
            print("  3. Run: python config_manager.py --auto-detect")
            print("  4. Manually edit config.json")
            print()
            print("Your MT5 Files directory is typically at:")
            print(r"  C:\Users\YourName\AppData\Roaming\MetaQuotes\Terminal\HASH\MQL5\Files")
            print()
            print(r"To find it: Open MT5 -> File -> Open Data Folder -> navigate to MQL5\Files")
            print()
            sys.exit(1)
    ...
    def _build_dl_model(self, model_type: str, input_shape: Tuple[int, int], hp: Optional[kt.HyperParameters] = None) -> Model:
        ...
        if model_type == 'lstm':
            ...
            x = layers.Conv1D(filters=conv_filters, kernel_size=3, dilation_rate=1, padding='causal', activation='relu')(x)
            x = layers.Conv1D(filters=conv_filters, kernel_size=3, dilation_rate=2, padding='causal', activation='relu')(x)
            x = layers.Conv1D(filters=conv_filters, kernel_size=3, dilation_rate=4, padding='causal', activation='relu')(x)
            x = x[:, -1, :]
        else:
    ...
    def train_model_multitimeframe(self, force_retrain: bool = False) -> None:
        ...
        # FIXED: Copy 1H scalers and models to base names for backward compatibility with backtest
        # This allows single-timeframe operations to work with multi-timeframe trained models
        ...
        try:
            ...
            for model_type in self.ensemble_model_types:
                ...
                ext = '.keras' if model_type in ['lstm', 'transformer', 'tcn'] else '.pkl'
    ...
    def load_model_assets(self) -> bool:
        ...
        try:
            ...
            # FIXED: Try to load base scaler first, fallback to multi-TF scalers if needed
            ...
            for model_type in self.ensemble_model_types:
                ...
                # FIXED: Auto-detect multi-timeframe model files if base doesn't exist
    ...
    def load_model_assets_multitimeframe(self) -> bool:
        ...
        for tf_name in timeframe_list:
            ...
            for model_type in self.ensemble_model_types:
                ...
                if model_type in ['lstm', 'transformer', 'tcn']:
                    model_path = self._get_model_path(model_type, model_index).replace('.h5', f'_{tf_name}.h5')
                    if os.path.exists(model_path):
                        models[model_name] = load_model(model_path, custom_objects={'TransformerBlock': TransformerBlock})
        ...
        for f in all_model_files:
            ...
            if len(parts) >= 4:
                ...
                try:
                    ...
                    # FIXED: Also detect multi-timeframe models (not just skip them)
                    # Remove timeframe suffix if present to get the base index
                    if index_part in ['1H', '4H', '1D']:
                        # This is a multi-TF model, but we still want to know the type exists
                        # The index is in parts[3] before the timeframe
                        # e.g. model_USDJPY_lgbm_0_1H.pkl -> parts[3] is '0' followed by '1H'
                        # Actually parts[3] would be '0' and parts[4] would be '1H'
                        # Let me re-parse this...
                        if len(parts) >= 5:
                            # Format: model_SYMBOL_TYPE_INDEX_TIMEFRAME.ext
                            # e.g. model_USDJPY_lgbm_0_1H.pkl
                            try:
                                model_index = int(parts[3])
                                if model_type in ['lstm', 'transformer', 'tcn', 'lgbm']:
    ...
    def run_prediction_cycle(self) -> None:
        """
        Run prediction cycle (OLD METHOD - uses scaling).
        For better results, use run_prediction_cycle_multitimeframe() instead.
        """
        print("\n" + "="*60)
        ...
        if final_tab_cols:
            ...
            if last_row.isna().any():
                ...
                df_tabular.fillna(method='ffill', inplace=True)
        ...
        for tf_name, steps in timeframes.items():
            ...
            # NEW: change percentage for the EA
    ...
    def run_prediction_cycle_multitimeframe(self) -> None:
        """
        Run prediction cycle using timeframe-specific models.
        This is the RECOMMENDED method for accurate predictions.
        """
        print("\n" + "="*60)
    ...
    def update_ensemble_weights(self) -> None:
        ...
        inverse_errors = [1.0 / max(err, 1e-8) for err in avg_errors]
        total_inverse = sum(inverse_errors)
        
        if total_inverse == 0:
            print("   WARNING: All inverse errors are zero, keeping equal weights")
            return
        
        new_weights = [ie / total_inverse for ie in inverse_errors]
    ...
    def run_backtest_generation(self) -> None:
        ...
        print("\n" + "="*60)
        print("BACKTEST MODE: Generating Historical Predictions")
        print("="*60 + "\n")
...
def main():
    ...
    print("""
    ================================================================
       Hybrid Ensemble MT5 Predictor v6.5.2 (Stable)
       Feat: Transformer, TCN, LightGBM & Multi-Timeframe Support
       TensorFlow Optimized - 2-3x Faster Predictions
    ================================================================
    """)
    
    # Setup argument parser
    parser = argparse.ArgumentParser(description="Hybrid Ensemble MT5 Predictor v6.5.2 (Stable)")
    ...
    p_train.add_argument(
        ...
        choices=['lstm', 'transformer', 'tcn', 'lgbm'],
        help="List of model types for the ensemble."
    )
    
    # Train multi-timeframe mode (NEW - recommended)
    ...
    p_predict.add_argument('--models', nargs='+', choices=['lstm', 'transformer', 'tcn', 'lgbm'], 
    ...
    # Predict multi-timeframe mode (NEW - recommended)
    ...
    p_predict_mtf.add_argument('--models', nargs='+', choices=['lstm', 'transformer', 'tcn', 'lgbm'], 
                              help="Override automatic model detection (optional)")
