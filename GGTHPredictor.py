...
Copyright 2026
Version: 6.6.0 - INTEGRATED MACRO & ANTI-CHEAT
...
- Maintained all v6.5.2 performance optimizations
"""
...
class UnifiedLSTMPredictor:
    def __init__(self, symbol: str = "EURUSD", related_symbols: Optional[List[str]] = None,
        ...
        self.kalman_config = {"1H": {"Q": 0.00001, "R": 0.01}, "4H": {"Q": 0.00005, "R": 0.02}, "1D": {"Q": 0.0001, "R": 0.05}}
    ...
    def get_market_context(self, df_main, df_dxy, df_spx):
        ...
        return {
            ...
            "z_score": round(z_score_risk, 2),
            "dxy_corr": round(current_corr, 4)
        }
    ...
    def train_model_multitimeframe(self, force_retrain: bool = False) -> None:
        ...
        # FIXED: Copy 1H scalers and models to base names for backward compatibility
        ...
        for f in all_model_files:
            ...
            if len(parts) >= 4:
                ...
                try:
                    ...
                    # FIXED: Also detect multi-timeframe models
    ...
    def run_prediction_cycle(self):
        ...
        # Download macro data
        df_h1, df_h4, df_d1 = self.download_data(300)
        df_dxy = pd.DataFrame(mt5.copy_rates_from_pos(self.dxy_symbol, mt5.TIMEFRAME_H1, 0, 300))
        df_spx = pd.DataFrame(mt5.copy_rates_from_pos(self.spx_symbol, mt5.TIMEFRAME_H1, 0, 300))
...
def main():
    ...
    print("""
    ================================================================
       Hybrid Ensemble MT5 Predictor v6.6.0 (With Macro Integration)
    ...
    parser = argparse.ArgumentParser(description="Hybrid Ensemble MT5 Predictor v6.6.0")
