...
//|                                      PredictionFinal.mq5        |
//|                                              Jason Rusk          |
//|                       For Multi-Timeframe Predictor v8.1        |
//+------------------------------------------------------------------+
...
#property description "v8.1: Added Averaging Down & Profit Protection"
...
input double InpAvgLevel1Lots = 1.0;                   // Level 1: Lot Size
input int InpAvgLevel1Pips = 20;                       // Level 1: Pips Against Position
input double InpAvgLevel2Lots = 1.0;                   // Level 2: Lot Size
input int InpAvgLevel2Pips = 30;                       // Level 2: Pips Against Position
input double InpAvgLevel3Lots = 1.0;                   // Level 3: Lot Size
...
input double InpProfitTargetAmount = 100.0;            // Profit Target ($) to Close All
...
//+------------------------------------------------------------------+
bool GetFirstPositionInfo(double &entry_price, long &pos_type, datetime &open_time)
{
...
void CheckAveragingDown()
{
   ...
   if(!GetFirstPositionInfo(entry_price, pos_type, open_time))
      return;
...
bool ExecuteAveragingOrder(int level, double lots)
{
   ...
   if(g_avg_state.original_position_type == POSITION_TYPE_BUY)
   {
      ...
      result = g_trade.Buy(lots, InpSymbol, ask, 0, 0, comment);
      
      if(result && InpShowDebug)
         Print("✓ BUY averaging order Level ", level, ": ", lots, " lots at ", ask);
   }
   else if(g_avg_state.original_position_type == POSITION_TYPE_SELL)
   {
      double bid = SymbolInfoDouble(InpSymbol, SYMBOL_BID);
      result = g_trade.Sell(lots, InpSymbol, bid, 0, 0, comment);
      
      if(result && InpShowDebug)
         Print("✓ SELL averaging order Level ", level, ": ", lots, " lots at ", bid);
   }
...
void UpdateDisplay()
{
   ...
   if(!InpUseTrendFilter && !InpUseRSIFilter && !InpUseMarketContextVeto && 
      !InpUseAveragingDown && !InpUseProfitProtection)
      filters += "None";
